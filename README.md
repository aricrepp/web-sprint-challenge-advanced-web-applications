...
There are multiple patterns like sanitization, stringifying, and one that we learned for frontend was using JSON tokens to authorize use of a protected database endpoint.

3. Describe how web servers work.

Web Servers comunicate back and forth with the client, your computer. So say you want to access Facebook. You would send a request for facebook.com using the facebook DNS which would then send for access to facebook to the nearest server where Facebook is stored. That server would then send back a packet of data, saying it was a good send 200, which would be the home page of Facebook. You then log in sending another response to access Facebooks protected endpoints. The server verifies you as a registered user and sends a token confirming log in and allowing you access.
